...
int main()
{
    ...
    Shader world_s("world_vert.vert", "world_frag.frag");
    Shader world_fvw_s("world_fvw_vert.vert", "world_fvw_frag.frag");
    Shader world_cam_s("world_cam_vert.vert", "world_cam_frag.frag");
    ...
    unsigned int WorldTex = LoadTexture("Images/In/room.jpg", GL_RGB, GL_RGB, 0);
    world_s.use();
    world_s.setInt("image", 0);
    #pragma endregion

    #pragma region Fvw Settings
    fvw_win = CreateWindow("fvw", 480, 360);
    GLEWInit();

    Shader fvw_s("fvw_vert.vert", "fvw_frag.frag");

    //plane data
    unsigned int fvwVAO, fvwVBO, fvwEBO;
    glGenVertexArrays(1, &fvwVAO);
    glGenBuffers(1, &fvwVBO);
    glGenBuffers(1, &fvwEBO);
    glBindVertexArray(fvwVAO);
    glBindBuffer(GL_ARRAY_BUFFER, fvwVBO);
    glBufferData(GL_ARRAY_BUFFER, sizeof(planeVertices), planeVertices, GL_STATIC_DRAW);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, fvwEBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices_plane), indices_plane, GL_STATIC_DRAW);
    ...
    TexBuffer1 = LoadTexture("Images/In/flower.jpg", GL_RGB, GL_RGB, 1);
    TexBuffer2 = LoadTexture("Images/In/flower2.jpg", GL_RGB, GL_RGB, 2);
    fvw_s.use();
    ...
    //glBindVertexArray(0);
    //
    //TexBuffer1 = LoadTexture("Images/In/room.jpg", GL_RGB, GL_RGB, 0);
    //image1_s.use();
    ...
    //glBindVertexArray(0);
    //
    //TexBuffer2 = LoadTexture("Images/In/kouen.jpg", GL_RGB, GL_RGB, 0);
    //image2_s.use();
    ...
    while (!glfwWindowShouldClose(world_win) && !glfwWindowShouldClose(fvw_win)/* && !glfwWindowShouldClose(image1_win) && !glfwWindowShouldClose(image2_win)*/)
    {
        ...
        mat4 projection_fvw = perspective(radians(fov), (float)480 / 360, 0.1f, 100.0f);
